...
* 就是通过script标签调用后台封装的一个js文件，那里面执行了html里面的回调函数，并且回传了数据
...
通过 <script> 标签，请求接口数据：
<script src="http://ajax.frontend.itheima.net:3006/api/jsonp?callback=success&name=zs&age=20"> --还可以定义要调用的回调函数，传参。
</script>
```
